...
% params.addParamValue('lineProps', '-k', @(x) ischar(x) | iscell(x)); % for matlab old version
% params.addParamValue('transparent', true, @(x) islogical(x) || x==0 || x==1);  % for matlab old version
% params.addParamValue('patchSaturation', 0.2, @(x) isnumeric(x) && x>=0 && x<=1);  % for matlab old version
